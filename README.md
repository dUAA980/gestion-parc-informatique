...
- `utilisateurs` 1 --- * `audit_logs` : un utilisateur peut generer plusieurs actions journalisees.
- `utilisateurs` 1 --- * `mouvements` : un mouvement est enregistre par un utilisateur.
- `locaux_it` 1 --- * `baies_it` : un local IT contient plusieurs baies.
- `baies_it` 1 --- * `equipements_baies` : une baie peut contenir plusieurs equipements.
- `mouvements` 1 --- * `equipements_baies` : un mouvement peut etre reference dans plusieurs affectations de baies.
- `locaux_it` 1 --- * `materiel_it` : un materiel IT peut etre rattache directement a un local.
- `baies_it` 1 --- * `materiel_it` : un materiel IT peut aussi etre rattache a une baie.
- `mouvements` 1 --- * `mouvements_historique` : lors d'une suppression, les donnees sont archivees dans l'historique.
- `mouvements` 1 --- * `dechets` : une sortie en mode dechet est enregistree dans la table `dechets`.
...
utilisateurs ────< audit_logs
	│
	└────< mouvements ────< equipements_baies >─── baies_it ────< locaux_it
			    │
			    ├────< mouvements_historique
			    └────< dechets

locaux_it ────< materiel_it
baies_it   ────< materiel_it
```
